...
# 已支持的功能：
  ...
  7、支持插件使用宿主程序主题、系统主题、插件自身主题以及style、轻松支持皮肤插件
...
# 暂不支持的功能：
  ...
  1、暂不支持插件资源文件中直接引用共享依赖库中的资源。但是支持间接使用，即支持直接通过依赖库中的R去引用。
  
  2、插件中发送notification时，不支持使用插件自己的资源。
  
  3、插件Activity切换动画不支持使用插件自己的资源。
  
  4、不支持插件申请权限，权限必须预埋到宿主中。
  
  5、不支持第三方app试图唤起插件中的组件时直接使用插件app的Intent。即插件app不能认为自己是一个正常安装的app。第三方app要唤起插件中的静态组件时必须由宿主程序进行桥接。
